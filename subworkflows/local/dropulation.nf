...
workflow dropulation_workflow {
    ...
    FILTER_DGE(
        donorCellBarcodes.map { m, f -> tuple(m + [id: m.id + ".donors"], f) },
        noMetaChannelHelper(dge).collect(),
        noMetaChannelHelper(dgeSummary).collect(),
    )
    CREATE_METACELLS(
        donorAssignments.map { m, f -> tuple(m + [id: m.id + ".donors"], f, []) },
    ...
    donorAssignmentTearSheet.combine(fullDropulationDir).subscribe { file, dir ->
        if (params.email) {
            sendMail(
                subject: "Dropulation summary for ${params.library}",
                body: "Dropulation summary for ${params.library} in ${dir}",
                to: params.email,
                attach: file,
            )
        }
